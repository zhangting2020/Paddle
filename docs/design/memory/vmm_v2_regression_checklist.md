...
| 日期/时间 | 范围 | 日志/验证 | 结论 |
|-----------|------|-----------|------|
| 2026-06-24 00:42-00:44 | `4bf6c67d27` fake 清理后的 IPC 与 targeted C++ 验证 | C++: `ctest --test-dir build -R '^(cuda_virtual_mem_allocator_v2_test\|vmm_auto_growth_best_fit_allocator_v2_test\|vmm_auto_growth_best_fit_multi_pool_allocator_v2_test)$'`; Python V2 IPC smoke | PASS；C++ targeted tests 3/3 PASS；Python V2 IPC smoke 在 `FLAGS_use_vmm_auto_growth_best_fit_allocator_v2=1` 下通过，覆盖 `_share_cuda/_new_shared_cuda` 基础 roundtrip、slice export 的 data-size/offset、`multiprocessing.reductions._reduce_lodtensor`、`build_reduce_scatter_buffer` 的 `FusedCommBuffer.param_buffer_ipc_meta`、`FusionStorage/FusionStorageHelper`。注意：`test/legacy_test/test_cuda_vmm_memory.py` 整体不能直接带 V2 flag 跑，因为其 `setUp()` 会打开 V1 flag 并与 V2 互斥；本轮使用直接 V2 smoke 覆盖真实 IPC 导出路径 |
| 2026-06-24 00:44 | `4bf6c67d27` fake 清理后的 allocator mapped-free 性能复测 | `/work/dev_tool/Paddle/tmp/vmm_v2_allocator_perf_final_20260624/summary.md` | PASS；正式 benchmark 只覆盖 h=2/h=16、fixed/random，不再包含 fake/legacy 模式；每组 `mapped_free cnt=640`；h2 fixed/random mapped_free 总耗时 `0.120ms/0.083ms`，h16 fixed/random 为 `0.085ms/0.073ms`；h2/h16 ratio fixed `1.41x`、random `1.14x`，绝对耗时均为 0.1ms 量级，无旧版 parts 热路径数量级差异 |
