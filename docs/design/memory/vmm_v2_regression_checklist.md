...
| 日期/时间 | 范围 | 日志/验证 | 结论 |
|-----------|------|-----------|------|
| 2026-06-23 23:31 | `112b0b2f5b` allocator mapped-free 热路径性能回归 | `/work/dev_tool/Paddle/tmp/vmm_v2_allocator_perf_112b0b2f5b_20260623_233147/summary.md` | PASS；脚本 `tools/vmm_v2_allocator_perf_benchmark.py` 跑 h=2/h=16、legacy/optimized/lazy、fixed/random 共 12 组；每组 `mapped_free cnt=640`，`avg src/alloc/remainder parts=0.0`、`max src parts=0`，确认 normal mapped-free reuse 热路径不再维护 `BlockV2::parts_`；h2 mapped_free 总耗时 `0.111-0.155ms`，h16 为 `0.120-0.194ms`，h2/h16 无数量级差异；ratio：fixed `h2 legacy / h16 legacy = 0.86x`，random `=1.19x` |
