 ...
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    if (fuse_relu) {
      ...
      depthwiseConv(dev_ctx, *input, filter, strides, paddings,
 ...
 public:
  void Compute(const framework::ExecutionContext& context) const override {
    ...
    const std::string padding_algorithm =
        context.Attr<std::string>("padding_algorithm");
    const std::string data_format = context.Attr<std::string>("data_format");
    const framework::DataLayout data_layout = framework::StringToDataLayout(data_format);
    ...
    if (input_grad) {
      ...
      if (fuse_relu) {
        ...
        depthwiseConvInputGrad(dev_ctx, *input, filter,
                               *output_grad, strides, paddings,
