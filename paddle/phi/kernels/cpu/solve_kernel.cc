...
static void linalg_solve(const Context& dev_ctx,
  ...
  if (!left) {
    // right solve does not support vector rhs
  ...
  Tensor x_tmp, y_tmp;
  if (!left) {
    // transpose last two dims for right solve
    ...
    y_in = y_tmp;
    // vector rhs is already prevented
  }
  ...
  if (is_vector) {
    ...
    phi::Unsqueeze<T, Context>(dev_ctx, y_in, {-1}, &tmp_y, nullptr);
  } else {
