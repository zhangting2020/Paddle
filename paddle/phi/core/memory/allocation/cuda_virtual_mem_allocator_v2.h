...
class CUDAVirtualMemAllocatorV2 : public Allocator {
 public:
  ...
  void advance_tail(size_t bytes) { virtual_mem_alloced_offset_ += bytes; }

  void UnmapHandle(VmmDevicePtr ptr, size_t size);
  void MapHandlesToVA(VmmDevicePtr ptr, const std::vector<VmmAllocHandle>& hs);
  bool CollectAllocationParts(void* base_ptr, std::vector<BlockPart>* parts) const;
 ...
 private:
  void InitOnce();
  void RegisterHandles(void* base_ptr, const std::vector<BlockPart>& parts);
  ...
  mutable std::unordered_map<void*, std::vector<BlockPart>> base_ptr_parts_map_;
