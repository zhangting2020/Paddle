...
phi::Allocation* CUDAVirtualMemAllocatorV2::AllocateImpl(size_t size) {
  ...
  for (size_t i = 0; i < num_handles; ++i) {
    ...
    parts.push_back(BlockPart{
        std::make_shared<VmmChunkMeta>(VmmChunkMeta{
  ...
  virtual_mem_alloced_offset_ += aligned;
...
void CUDAVirtualMemAllocatorV2::FreeImpl(phi::Allocation* allocation) {
  ...
  for (const auto& part : parts) {
    ...
    // TODO(zhangting35): Once remap / IPC allows multiple BlockParts to share
    // one VmmChunkMeta handle, release should move to chunk lifetime
    // management instead of immediate cuMemRelease here.
...
bool CUDAVirtualMemAllocatorV2::CollectAllocationParts(
    void* base_ptr, std::vector<BlockPart>* parts) const {
  std::lock_guard<std::mutex> guard(base_ptr_parts_mu_);
  auto it = base_ptr_parts_map_.find(base_ptr);
...
void CUDAVirtualMemAllocatorV2::RegisterHandles(
    void* base_ptr, const std::vector<BlockPart>& parts) {
  std::lock_guard<std::mutex> guard(base_ptr_parts_mu_);
