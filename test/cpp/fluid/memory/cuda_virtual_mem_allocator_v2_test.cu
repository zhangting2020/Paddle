...
TEST(CUDAVirtualMemAllocatorV2, CollectCompactCandidatesDelegatesToBackingMap) {
  CUDAVirtualMemAllocatorV2 allocator(
      phi::GPUPlace(), 2UL << 20, PoolType::kLarge);
  ...
  auto candidates =
      allocator.CollectCompactCandidates({{source_va, allocator.handle_size()}},
                                         {{target_va, allocator.handle_size()}},
                                         allocator.handle_size());
  ASSERT_EQ(candidates.source_pages.size(), 1UL);
  ASSERT_EQ(candidates.target_pages.size(), 1UL);
  EXPECT_EQ(candidates.source_pages[0].va, source_va);
  EXPECT_EQ(candidates.target_pages[0].va, target_va);
}

TEST(CUDAVirtualMemAllocatorV2, RestoreRemapSourceMappingSkipsInvalidMeta) {
  CUDAVirtualMemAllocatorV2 allocator(
      phi::GPUPlace(), 2UL << 20, PoolType::kLarge);

  auto meta = std::make_shared<VMMHandleMeta>(
      0, allocator.handle_size(), static_cast<VMMAllocHandle>(0x301), 0);
  EXPECT_EQ(
      allocator.RestoreRemapSourceMapping(
          static_cast<VMMAllocHandle>(0x301), nullptr, allocator.handle_size()),
      CUDAVirtualMemAllocatorV2::RestoreRemapSourceResult::kSkipped);
  EXPECT_EQ(allocator.RestoreRemapSourceMapping(
                meta->handle(), meta, allocator.handle_size()),
            CUDAVirtualMemAllocatorV2::RestoreRemapSourceResult::kSkipped);
}

TEST(CUDAVirtualMemAllocatorV2, DestroyStagedSyntheticAllocationIgnoresNull) {
  CUDAVirtualMemAllocatorV2 allocator(
      phi::GPUPlace(), 2UL << 20, PoolType::kLarge);

  EXPECT_NO_THROW(allocator.DestroyStagedSyntheticAllocation(nullptr));
}
